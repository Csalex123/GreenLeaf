...
$pesquisar = utf8_decode($_GET['pesquisar']);
...
$resultado_publicacao = "SELECT *,usuario.nome as nomeUsuario, usuario.foto as fotoUsuario, publicacao.id_publicacao as id_publicacao FROM publicacao inner join usuario on publicacao.id_usuario = usuario.id_usuario   where  tema like '%$pesquisar%' ORDER BY data_hora DESC";
...
if (mysqli_num_rows($resultado_publi) == 0) {
	echo "<script>alert('Não existe nenhum filtro com este nome $pesquisar'); history.back(); </script>";
}
...
<div>
		<form class="navbar-form navbar-right"  action="Filtro.php" method="GET" style="margin-right: 20px; ">
          <div class="input-group ">
            <input type="text" class="form-control" style="float: center; " name="pesquisar"  autocomplete="" id="procurar" placeholder="Filtre por assunto">
...
<div class="container col-sm-7" id="publicacao" style="margin-bottom:  20px; margin-top: -14px;">
	...
	<tr>
		...
		<td colspan="3" style="font-size: 1.4em; color: #1E90FF;"><br><b>Titulo:</b> 
		...
		<tr >
			...
			<tr>
				...
				<td colspan="3">
					...
					$query = "SELECT COUNT(id_publicacao) as total FROM comentario WHERE comentario.id_publicacao =". $dado['id_publicacao']; 
					...
					while($dado2 = $con2->fetch_array()){ 
						...
						<table border="0" style="width:70%; float: right;" >
							...
							<tr>
								...
								<td>
									<b ><?php  echo utf8_encode (ucwords(($dado2["nomeUsuario"]))); ?>
									</b>
								</td>
								...
								<td>
									...
									if ($id_usuario == $id_usuario_comentario) {
										...
										echo "<form name='seuformulario' method='GET' action='forum/deletarcomentariofiltro.php'>
					...
					<table border="0" style="width:90%; float: right;">
						...
						<tr>
							...
							<td>
								...
								<form action="forum/inserircomentariofiltro.php"  method="post" onsubmit="alterarValor();">
